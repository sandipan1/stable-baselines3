...
class EvalCallback(BaseCallback):
    ...
    """
    def __init__(self, eval_env, n_eval_episodes=5, eval_freq=1000, model_dir=None):
        super(EvalCallback, self).__init__()
        self.eval_env = eval_env
        self.n_eval_episodes = n_eval_episodes
        self.eval_freq = eval_freq
        self.model_dir = model_dir
        self.save_best_model = os.path.join(model_dir, 'best_model')
        self.save_latest_model = os.path.join(model_dir, "latest_model")
        self.best_mean_reward = -np.inf
    ...
    def _on_step(self):
        ...
        # and update self.best_mean_reward
            if mean_reward > self.best_mean_reward:
                self.best_mean_reward = mean_reward
                self.model.save(self.save_best_model)
            if self.verbose > 0:
                print("Saving new best model at {} timesteps".format(self.num_timesteps))
                print("Saving new best model to {}.zip".format(self.save_best_model))
